...
const addGuest = async (formData: GuestFormData): Promise<Guest> => {
  ...
  const restoreGuest = async (guestId: string) => {
    ...
    try {
      const { error } = await supabase
        .from('invitati')
        .update({ 
          note: buildNote({ allergies: null, deleted_at: null }),
          bomboniera_assegnata: false  // Reset bomboniera quando si ripristina (torna a pending)
        })
        .eq('unita_invito_id', unitId);
  ...
  const revertGuestOnly = async (guestId: string) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({ 
          confermato: false,
          bomboniera_assegnata: false  // Reset bomboniera quando si riporta a pending
        })
        .eq('unita_invito_id', unitId)
        .eq('is_principale', true);
  ...
  const updateCompanionStatus = async (guestId: string, companionId: string, status: GuestStatus) => {
    ...
    try {
      if (status === 'confirmed' || status === 'pending') {
        const updateData: any = { confermato: status === 'confirmed' };
        
        // Se passiamo a pending, resettiamo anche la bomboniera
        if (status === 'pending') {
          updateData.bomboniera_assegnata = false;
        }

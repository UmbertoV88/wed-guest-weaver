...
export const bombonieraApi = {
  ...
  /**
   * Conta le bomboniere assegnate per categoria (per suddivisione costi sposo/sposa)
   * @returns Oggetto con conteggi per categoria (solo bomboniere assegnate)
   */
  async getAssignedCountByCategory(): Promise<{
    familyHis: number;
    familyHers: number;
    friends: number;
    colleagues: number;
    ...
    try {
      ...
      if (error) {
        console.error('Errore conteggio bomboniere per categoria:', error);
        throw error;
      }
      ...
      console.error('Errore bombonieraApi.getAssignedCountByCategory:', error);
      return {
        familyHis: 0,
        familyHers: 0,
        friends: 0,
...
export const guestsApi = {
  /**
   * Conta il numero totale di ospiti confermati
   * @returns Numero di ospiti con confermato = true
   */
  async getConfirmedCount(): Promise<number> {
    try {
      ...
      if (error) {
        console.error('Errore conteggio ospiti confermati:', error);
        throw error;
      }
      
      return count || 0;
    } catch (error) {
      console.error('Errore guestsApi.getConfirmedCount:', error);
      return 0;
    }
  },

  /**
   * Conta gli ospiti confermati per categoria (per suddivisione costi sposo/sposa)
   * @returns Oggetto con conteggi per categoria
   */
  async getConfirmedCountByCategory(): Promise<{
    familyHis: number;
    familyHers: number;
    friends: number;
    colleagues: number;
    ...
    try {
      ...
      if (error) {
        console.error('Errore conteggio ospiti per categoria:', error);
        throw error;
      }
      ...
      console.error('Errore guestsApi.getConfirmedCountByCategory:', error);
      return {
        familyHis: 0,
        familyHers: 0,
        friends: 0,

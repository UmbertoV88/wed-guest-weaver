...
const CategoryManager: React.FC<CategoryManagerProps> = ({
  ...
  // Effect per caricare il conteggio ospiti confermati
  useEffect(() => {
    ...
    // Aggiorna ogni 10 secondi
    const interval = setInterval(loadConfirmedGuestsCount, 10000);
    return () => clearInterval(interval);
  }, [categories]);
      ...
      >
        {sortedCategories.map((category) => {
          ...
          const actualSpent = categoryVendors.reduce((sum, vendor) => {
            ...
            // Per Bomboniere e Sala Ricevimento, calcola il costo totale dinamico
            let vendorTotalCost = unitCost;
            if (isBombonieraCategory && bombonieraCount > 0) {
              vendorTotalCost = unitCost * bombonieraCount;
            } else if (isSalaRicevimentoCategory && confirmedGuestsCount > 0) {
              ...
              <AccordionTrigger className="hover:no-underline px-6 py-4 hover:bg-muted/50">
                <div className="flex items-center justify-between w-full pr-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-left">
                      <div className="flex items-center gap-2">
                        ...
                        {/* Badge Sala Ricevimento - SOLO se categoria è "Sala Ricevimento" */}
                        {category.name.toLowerCase().includes('sala') &&
                          category.name.toLowerCase().includes('ricevimento') &&
                  ...
                  <div className="flex items-center gap-6">
                    {getStatusBadge(actualSpent, category.budgeted)}
                            ...
                            return (
                              <div className="space-y-2">
                                {categoryVendors.map((vendor: any) => {
                                  ...
                                  // Calcola il costo totale dinamico per categorie speciali
                                  let total = unitCost;
                                  if (isBombonieraCategory && bombonieraCount > 0) {
                                    total = unitCost * bombonieraCount;
                                  } else if (isSalaRicevimentoCategory && confirmedGuestsCount > 0) {
                                    total = unitCost * confirmedGuestsCount;
                                  }
